...
type ServiceCollectionExtension =
    ...
    static member AddSnitch(services: IServiceCollection, appName: string) =
        ...
        services.AddSingleton<ISnitch>(Slack(httpClient, appName, slackHookUrl))
        |> ignore

        services.AddSingleton<ConcurrentDictionary<string, Except>>(ConcurrentDictionary<string, Except>())
        |> ignore

        services.AddHostedService<SnitchService>() |> ignore

        SnitchServiceLocator.initialize (services.BuildServiceProvider())

    [<Extension>]
    static member UseSnitch(app: IApplicationBuilder) = app.UseMiddleware<SnitchMiddleware>()
